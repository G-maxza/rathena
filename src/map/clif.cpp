	...
	case BL_MOB:
		{
			...
			// Chat Clone
			if (md->chat_clone_id)
				clif_dispchat((struct chat_data*)map_id2bl(md->chat_clone_id), sd->fd);
...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t)
{
	...
	WBUFL(buf,4) = ((mapdata->flag[MF_PVP] || (sd && sd->duel_group > 0))<<0)| // PARTY - Show attack cursor on non-party members (PvP)
		...
		((mapdata->flag[MF_NOLOCKON] || mapdata_flag_vs(mapdata) || mapdata->flag[MF_PK] || (sd && sd->duel_group > 0))<<4)| // DISABLE_LOCKON - Only allow attacks on other players with shift key or /ns active
...
void clif_showvendingboard( struct map_session_data& sd, enum send_target target, struct block_list* tbl ){
	if( tbl == nullptr ){
		tbl = &sd.bl;
		target = AREA_WOS;
	}
...
void clif_party_info( struct party_data& party, struct map_session_data* sd ){
	send_target target;
...
void clif_party_message( struct party_data& party, uint32 account_id, const char* mes, int len ){
	struct map_session_data* sd = party_getavailablesd( &party );
...
void clif_spiritball( struct block_list *bl, struct block_list* target, enum send_target send_target ){
	...
	switch( bl->type ){
		case BL_PC:
			p.num = ( (struct map_session_data*)bl )->spiritball;
			break;
		case BL_HOM:
...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if( sd->state.changemap ) {// restore information that gets lost on map-change
		...
		// VIP System
		if (pc_isvip(sd) && !sd->state.message_vip)
			pc_displaymessage_vip(sd);
#endif
...
//	safestrncpy(sender, RFIFOCP(fd, 28), NAME_LENGTH);
	...
	if( zeny > 0 ){
		if( (mail_setitem(sd,0,(uint32)zeny) != MAIL_ATTACH_SUCCESS ) || (zeny > 100000000))
		{
			clif_Mail_send(sd,WRITE_MAIL_FAILED);
			return;
		}
...
void clif_buyingstore_entry( struct map_session_data& sd, struct block_list* tbl ){
...
/// 0816 <account id>.L
void clif_buyingstore_disappear_entry( struct map_session_data& sd, struct block_list* tbl ){
...
/**    https://ronovelty.com/     **/
/***********************************/
void clif_connection_reward(struct map_session_data* sd, int time){
#if PACKETVER >= 20150513
	nullpo_retv( sd );
	...
	struct PACKET_ZC_CONNECTION_REWARD p = {};

	p.PacketType = HEADER_ZC_CONNECTION_REWARD;
	p.state = 1;
	if(pc_isvip(sd) && battle_config.feature_connection_reward_vip)
		p.type = 2;
	else
		p.type = 1;
	p.amount = sd->connection_reward_pts;
	p.time = MAX_CONNECTION_REWARD_TIME - time;

	clif_send( &p, sizeof( p ), &sd->bl, SELF );
#endif
}

void clif_parse_connection_reward(int fd,struct map_session_data *sd) {
#if PACKETVER >= 20150513
	const struct PACKET_CZ_CONNECTION_REWARD *p = (struct PACKET_CZ_CONNECTION_REWARD *)RFIFOP( fd, 0 );

	// Check if it s the good function called in the packet
	if (strncmp(p->feature_name, "GOLDPCCAFE", 10) != 0)
		return;
	
	if(sd->connection_script)
		script_free_code(sd->connection_script);
	
	if (!(sd->connection_script = parse_script("{ callfunc \"F_connection_reward\"; }", "script_connection_reward", 0, 0))) {
		ShowError("clif_parse_connection_reward: Failed to parse script 'F_connection_reward' (CID:%d).\n", sd->status.char_id);
		return;
	}
	run_script(sd->connection_script, 0, sd->bl.id, fake_nd->bl.id);
