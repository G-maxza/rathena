...
//	md->master_id = 0;
	...
	md->dynamic_dmg = 1; // [Start] Monster dynamic damage
...
// Chat Clone
int mob_clone_spawn_chat(struct map_session_data *sd, int16 m, int16 x, int16 y, char chat_title[], char chat_msg[]) {
	...
	db->lv = status_get_lv(&sd->bl);
	memcpy(status, &sd->base_status, sizeof(struct status_data));
	status->rhw.atk = status->rhw.atk2 = status->lhw.atk = status->lhw.atk2 = status->hp = status->max_hp = status->sp = status->max_sp =  1;
	status->mode = MD_NONE;
	memcpy(&db->vd, &sd->vd, sizeof(struct view_data));
	...
	cd = chat_createchat(&md->bl, chat_title, "", 1, false, 0, "", 0, 1, MAX_LEVEL);

	if (!cd)
		return 0;

	md->chat_clone_id = cd->bl.id;
	md->chat_clone_char_id = sd->status.char_id;
	safestrncpy(md->chat_clone_message, chat_msg, CHAT_SIZE_MAX);
	clif_dispchat(cd, 0);
