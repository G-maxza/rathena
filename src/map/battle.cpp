	...
	switch( attack_type ) {
		case BF_MAGIC:
			...
			if( tsd && !nk[NK_IGNOREDEFCARD] ) {
				...
				if(battle_config.max_size_def && tsd->indexed_bonus.subsize[sstatus->size] >= battle_config.max_size_def)
					tsd->indexed_bonus.subsize[sstatus->size] = battle_config.max_size_def;
				if(battle_config.max_size_def && tsd->indexed_bonus.subsize[SZ_ALL] >= battle_config.max_size_def)
					tsd->indexed_bonus.subsize[SZ_ALL] = battle_config.max_size_def;
				if(battle_config.max_race_def && tsd->indexed_bonus.subrace[sstatus->race] >= battle_config.max_race_def)
					tsd->indexed_bonus.subrace[sstatus->race] = battle_config.max_race_def;
				if(battle_config.max_race_def && tsd->indexed_bonus.subrace[RC_ALL] >= battle_config.max_race_def)
					tsd->indexed_bonus.subrace[RC_ALL] = battle_config.max_race_def;
				if(battle_config.max_class_def && tsd->indexed_bonus.subclass[sstatus->class_] >= battle_config.max_class_def)
					tsd->indexed_bonus.subclass[sstatus->class_] = battle_config.max_class_def;
				if(battle_config.max_class_def && tsd->indexed_bonus.subclass[CLASS_ALL] >= battle_config.max_class_def)
					tsd->indexed_bonus.subclass[CLASS_ALL] = battle_config.max_class_def;
...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	//PK damage rates
	if ((battle_config.pk_mode == 1 && map_getmapflag(bl->m, MF_PVP) > 0) || map_getmapflag(bl->m, MF_PK))
		damage = battle_calc_pk_damage(*src, *bl, damage, skill_id, flag);
	...
	if (bl->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		...
		// [Start] Reduces damage received for Dynamic value
		if (md && (md->dynamic > 1))
...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case SR_TIGERCANNON:
			{
				unsigned int hp = sstatus->max_hp * (10 + (skill_lv * 2)) / 500,
		...
		case DK_STORMSLASH:
			...
			skillratio += 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			if (sc && sc->data[SC_GIANTGROWTH] && rnd_chance(60, 100))
				skillratio *= 2;
			break;
		case DK_DRAGONIC_BREATH:
			//TODO: needs official HP/SP scaling [Muh]
			skillratio += -100 + 50 + 350 * skill_lv + 7 * sstatus->pow;
			skillratio += sstatus->max_hp / 50 + status_get_max_sp(src) / 50;
			if (sc && sc->data[SC_DRAGONIC_AURA])
				skillratio += -100 + 50 + 350 * skill_lv + 10 * sstatus->pow;
				skillratio += sstatus->max_hp / 70 + status_get_max_sp(src) / 30;
			RE_LVL_DMOD(100);
			break;
		...
		case IG_RADIANT_SPEAR:
			...
			if( sc != nullptr && sc->data[SC_SPEAR_SCAR])
		...
		case SHC_CROSS_SLASH:
			skillratio += -100 + 300 * skill_lv;
			skillratio += 5 * sstatus->pow;
			if( sc != nullptr && sc->data[SC_SHADOW_EXCEED])
		...
		case SS_FUUMASHOUAKU:
			...
			break;
		case SS_FUUMAKOUCHIKU:
			skillratio += -100 + 400 + 1800 * skill_lv;
		...
		case SS_KUNAIWAIKYOKU:
			...
			break;
		case SS_KUNAIKUSSETSU:
			skillratio += -100 + 450 + 400 * skill_lv;
			skillratio += pc_checkskill(sd, SS_KUNAIKAITEN) * 10 * skill_lv;
			skillratio += 10 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case SS_KUNAIKAITEN:
			skillratio += -100 + 200 + 1200 * skill_lv;
			skillratio += pc_checkskill(sd, SS_KUNAIWAIKYOKU) * 50 * skill_lv;
			skillratio += 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case SS_KAGEGISSEN:
			skillratio += -100 + 1000 + 1000 * skill_lv;
		...
		case SKE_MIDNIGHT_KICK:
			skillratio += -100 + 300 + 1600 * skill_lv;
			skillratio += pc_checkskill(sd, SKE_SKY_MASTERY) * 2.5 * skill_lv;
			skillratio += 5 * sstatus->pow;
		...
		case SKE_STAR_BURST:
			...
			break;
		case SKE_DAWN_BREAK:
			skillratio += -100 + 100 + 800 * skill_lv;
			skillratio += pc_checkskill(sd, SKE_SKY_MASTERY) * 2.5 * skill_lv;
			skillratio += 5 * sstatus->pow;
		...
		case SH_HOGOGONG_STRIKE:
			...
			break;
		case SH_CHUL_HO_SONIC_CLAW:
			skillratio += -100 + 2500 + 2000 * skill_lv + 50 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY) + 7 * sstatus->pow;
			if ((sd && pc_checkskill(sd, SH_COMMUNE_WITH_CHUL_HO)) || (sc && sc->data[SC_TEMPORARY_COMMUNION]))
				skillratio += 400 * skill_lv + 100 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY);
			RE_LVL_DMOD(100);
			break;
	}
	return skillratio;
			...
			default: {
				...
				switch(skill_id) {
					...
					case SS_SEKIENHOU:
						...
						skillratio += pc_checkskill(sd, SS_ANTENPOU) * 70 * skill_lv;
						skillratio += 10 * sstatus->spl;
						RE_LVL_DMOD(100);
						break;
					case SS_REIKETSUHOU:
						skillratio += -100 + 950 + 900 * skill_lv;
						skillratio += pc_checkskill(sd, SS_ANTENPOU) * 40 * skill_lv;
						skillratio += 10 * sstatus->spl;
						RE_LVL_DMOD(100);
						break;
					case SS_KINRYUUHOU:
						skillratio += -100 + 800 + 1500 * skill_lv;
						skillratio += pc_checkskill(sd, SS_ANTENPOU) * 15 * skill_lv;
						skillratio += 10 * sstatus->spl;
						RE_LVL_DMOD(100);
						break;
					case SS_ANKOKURYUUAKUMU:
					...
					case SH_HYUN_ROKS_BREEZE:
						...
						break;
					case SH_HYUN_ROK_CANNON:
						skillratio += -100 + 1450 + 2000 * skill_lv + 50 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY) + 7 * sstatus->spl;
						if ((sd && pc_checkskill(sd, SH_COMMUNE_WITH_HYUN_ROK)) || (sc && sc->data[SC_TEMPORARY_COMMUNION]))
							skillratio += 400 * skill_lv + 75 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY);
						RE_LVL_DMOD(100);
						break;
					...
					case SOA_TALISMAN_OF_BLUE_DRAGON:
						skillratio += -100 + 2100 + 2000 * skill_lv;
						skillratio += pc_checkskill(sd, SOA_TALISMAN_MASTERY) * 7.5 * skill_lv;
						skillratio += 5 * sstatus->spl;
					...
					case SOA_TALISMAN_OF_RED_PHOENIX:
						skillratio += -100 + 650 + 1600 * skill_lv;
						skillratio += pc_checkskill(sd, SOA_TALISMAN_MASTERY) * 7.5 * skill_lv;
						skillratio += 5 * sstatus->spl;
						if (sc && sc->data[SC_T_FIFTH_GOD])
							skillratio += 200 + 400 * skill_lv;
					...
					case IG_CROSS_RAIN:
						...
						break;
					case CD_ARBITRIUM:
						skillratio += -100 + (1000 + 10 * pc_checkskill(sd,CD_FIDUS_ANIMUS)) * skill_lv + 10 * sstatus->spl;
						RE_LVL_DMOD(100);
						break;
					case CD_ARBITRIUM_ATK:
						skillratio += -100 + (1750 + 50 * pc_checkskill(sd,CD_FIDUS_ANIMUS)) * skill_lv + 10 * sstatus->spl;
						RE_LVL_DMOD(100);
						break;
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, t_tick tick, int flag) {
	...
	if (sd) {
		...
		if( sc ){
			...
			if( sc->data[SC_SERVANTWEAPON] && sd->servantball > 0 && rnd() % 100 < (5 + sc->data[SC_SERVANTWEAPON]->val1) ){
			...
			// It has a success chance of triggering even tho the description says nothing about it.
			// TODO: Need to find out what the official success chance is. [Rytech]
			if( sc->data[SC_ABYSSFORCEWEAPON] && sd->abyssball > 0 && rnd() % 100 < 25 ){
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	// Chat Clone
	if (target->type == BL_MOB)
		if (((TBL_MOB*)target)->chat_clone_id)
			return -1;
	...
	if( mapdata_flag_vs(mapdata) || mapdata->flag[MF_PK] )
	{ //Check rivalry settings.
		...
		if( state&BCT_ENEMY && (battle_config.pk_mode || mapdata->flag[MF_PK]) && !mapdata_flag_gvg(mapdata) && s_bl->type == BL_PC && t_bl->type == BL_PC )
...
// rAthena
	...
	// Chat Clone
	{ "chat_clone_system",            	    &battle_config.chat_clone_system,         	    0,      0,      1,              },
	{ "chat_can_anywhere",            	    &battle_config.chat_can_anywhere,         	    0,      0,      1,              },
	{ "chat_clone_delay",           		&battle_config.chat_clone_delay,          	    0,      0,      INT_MAX,        },
	{ "chat_clone_zenycost",             	&battle_config.chat_clone_zenycost,             0,      0,      MAX_ZENY,       },
	{ "chat_msg_color",             		&battle_config.chat_msg_color,           0xFFE600,      0,      INT_MAX,        },
	...
	{ "feature.connection_reward",          &battle_config.feature_connection_reward,       1,      0,      1,              },
	{ "feature.connection_reward_timer",    &battle_config.feature_connection_reward_timer, 3600,   1,      3600,           },
	{ "feature.connection_reward_pts",      &battle_config.feature_connection_reward_pts,   1,      1,      9999,            },
	{ "feature.connection_reward_max",      &battle_config.feature_connection_reward_max,   300,    1,      9999,            },
	{ "feature.connection_reward_vip",      &battle_config.feature_connection_reward_vip,   0,      0,      1,              },
