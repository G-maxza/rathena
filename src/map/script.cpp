...
#define getunitdata_sub(idx__,var__) setd_sub_num(st,sd,name,(idx__),(var__),data->ref)
	...
	switch(bl->type) {
		case BL_MOB:
			...
			getunitdata_sub(UMOB_DYNAMIC_DAMAGE, md->dynamic_dmg); // [Start]
...
BUILDIN_FUNC(setunitdata)
{
	...
	case BL_MOB:
		...
		switch (type) {
			...
			case UMOB_DYNAMIC: md->dynamic = value; md->deflect = (value > 5) ? cap_value((value / 5), 1, 50) : 0; break; // [Start] Monster rank > 5 will grant deflect rate by (Rank / 5) (Maximum: 50%)
			case UMOB_DYNAMIC_DAMAGE: md->dynamic_dmg = value; break; // [Start] Monster dynamic damage
...
/**    https://ronovelty.com/     **/
/***********************************/
/// Return the connection reward point
///
/// getcoreward({<char_id>}) -> points
BUILDIN_FUNC(getcoreward)
{
	TBL_PC *sd;
	...
	script_pushint(st, sd->connection_reward_pts);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(setcoreward)
{
	struct map_session_data *sd;
	int points;
	...
	sd->connection_reward_pts = min(points, battle_config.feature_connection_reward_max);
...
// <--- [zBuffer] List of unit control commands
	...
	BUILDIN_DEF(getcoreward,"?"),
	BUILDIN_DEF(setcoreward,"i?"),
