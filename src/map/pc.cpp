...
void pc_reg_received(struct map_session_data *sd)
{
	...
	if(battle_config.feature_connection_reward){
		sd->connection_reward_pts = static_cast<int>(pc_readreg2(sd, "#coRewardPts"));
		sd->connection_reward_tick = static_cast<int>(pc_readreg2(sd, "#coRewardTime"));

		if(sd->connection_reward_tick > 0)
			sd->connection_reward_tick += time(NULL);
		else
			sd->connection_reward_tick = time(NULL) + battle_config.feature_connection_reward_timer;

		sd->connection_reward_timer = add_timer(gettick() + (sd->connection_reward_tick - time(NULL)) * 1000, feature_connection_reward_timeout, sd->bl.id, 0);
...
void pc_bonus2(struct map_session_data *sd,int type,int type2,int val)
{
	...
	case SP_SUBELE: // bonus2 bSubEle,e,x;
		...
		if (sd->indexed_bonus.subele_script[type2] > 95)
			sd->indexed_bonus.subele_script[type2] = 95;
		break;
	case SP_SUBRACE: // bonus2 bSubRace,r,x;
		PC_BONUS_CHK_RACE(type2,SP_SUBRACE);
		if(sd->state.lr_flag != 2)
			sd->indexed_bonus.subrace[type2]+=val;
		if (sd->indexed_bonus.subrace[type2] > 95)
			sd->indexed_bonus.subrace[type2] = 95;
		break;
	case SP_SUBCLASS: // bonus2 bSubClass,c,x;
		PC_BONUS_CHK_CLASS(type2,SP_SUBCLASS);
		if(sd->state.lr_flag != 2)
			sd->indexed_bonus.subclass[type2]+=val;
		if (sd->indexed_bonus.subclass[type2] > 95)
			sd->indexed_bonus.subclass[type2] = 95;
		break;
	...
	case SP_SUBSIZE: // bonus2 bSubSize,s,x;
		...
		if (sd->indexed_bonus.subsize[type2] > 95)
			sd->indexed_bonus.subsize[type2] = 95;
		break;
	case SP_WEAPON_SUBSIZE: // bonus2 bWeaponSubSize,s,x;
		...
		if (sd->indexed_bonus.weapon_subsize[type2] > 95)
			sd->indexed_bonus.weapon_subsize[type2] = 95;
		break;
	case SP_MAGIC_SUBSIZE: // bonus2 bMagicSubSize,s,x;
		...
		if (sd->indexed_bonus.magic_subsize[type2] > 95)
			sd->indexed_bonus.magic_subsize[type2] = 95;
		break;
 ...
 * @author [Shakto]
 **/
int pc_status_refresh(struct map_session_data *sd, va_list ap) {
	status_calc_pc(sd, SCO_FORCE);
 ...
 * @author [Shakto]
 **/
void pc_job_bonus(struct map_session_data *sd) {
	int idx = 0, i = 0;
...
/**    https://ronovelty.com/     **/
/***********************************/
TIMER_FUNC (feature_connection_reward_timeout){
	struct map_session_data *sd = map_id2sd(id);
	...
	sd->connection_reward_timer = INVALID_TIMER;

	if(!sd->connection_reward_pts)
		sd->connection_reward_pts = 0;

	// Add the number of point and check if VIP feature enabled
	if(pc_isvip(sd) && battle_config.feature_connection_reward_vip)
		sd->connection_reward_pts += 2 * battle_config.feature_connection_reward_pts;
	else
		sd->connection_reward_pts += battle_config.feature_connection_reward_pts;
	sd->connection_reward_pts = min(sd->connection_reward_pts, battle_config.feature_connection_reward_max);

	// Set the timer again
	sd->connection_reward_tick = time(NULL) + battle_config.feature_connection_reward_timer;
	sd->connection_reward_timer = add_timer(gettick() + battle_config.feature_connection_reward_timer * 1000, feature_connection_reward_timeout, sd->bl.id, 0);

	clif_connection_reward(sd, battle_config.feature_connection_reward_timer);
 ...
 * VIP System
 *------------------------------------------*/
void pc_displaymessage_vip(struct map_session_data *sd) {
	time_t now = time(NULL);
	int year, month, day, hour, minute, second;
	char atcmd_output[CHAT_SIZE_MAX],
	year_msg[TIMESTR_SIZE_MAX],
	month_msg[TIMESTR_SIZE_MAX],
	day_msg[TIMESTR_SIZE_MAX],
	hour_msg[TIMESTR_SIZE_MAX],
	minute_msg[TIMESTR_SIZE_MAX],
	second_msg[TIMESTR_SIZE_MAX];

	memset(atcmd_output, '\0', sizeof(atcmd_output));
	memset(year_msg, '\0', sizeof(year_msg));
	memset(month_msg, '\0', sizeof(month_msg));
	memset(day_msg, '\0', sizeof(day_msg));
	memset(hour_msg, '\0', sizeof(hour_msg));
	memset(minute_msg, '\0', sizeof(minute_msg));
	memset(second_msg, '\0', sizeof(second_msg));

	sd->state.message_vip = 1;

	split_time((int)(sd->vip.time - now), &year, &month, &day, &hour, &minute, &second);

	sprintf(atcmd_output, msg_txt(sd, 1530)); // Seu VIP é válido por
	sprintf(year_msg, year <= 1 ? msg_txt(sd, 1531) : msg_txt(sd, 1532), year); // %d anos, ou %d ano,
	sprintf(month_msg, month <= 1 ? msg_txt(sd, 1533) : msg_txt(sd, 1534), month); // %d meses, ou %d mês,
	sprintf(day_msg, day <= 1 ? msg_txt(sd, 1535) : msg_txt(sd, 1536), day); // %d dias, ou % dia,
	sprintf(hour_msg, hour <= 1 ? msg_txt(sd, 1537) : msg_txt(sd, 1538), hour); // %d horas, ou %d hora,
	sprintf(minute_msg, minute <= 1 ? msg_txt(sd, 1539) : msg_txt(sd, 1540), minute); // %d minutos, ou %d minuto
	sprintf(second_msg, second <= 1 ? msg_txt(sd, 1541) : msg_txt(sd, 1542), second); // %d segundos. ou %d segundo.
	if (year > 0)
		strcat(atcmd_output, year_msg);
	if (month > 0)
		strcat(atcmd_output, month_msg);
	if (day > 0)
		strcat(atcmd_output, day_msg);
	if (hour > 0)
		strcat(atcmd_output, hour_msg);
	if (minute > 0)
		strcat(atcmd_output, minute_msg);
	strcat(atcmd_output, second_msg);
...
void pc_collection_update(struct s_storage *stor, map_session_data &sd) {
	...
	if ((sd.state.collection_flag&PCCOLLECTION_LOAD && sd.collection_list.size() > 0) || sd.state.collection_flag&PCCOLLECTION_RECAL) {
		sprintf(output, msg_txt(&sd,(sd.state.collection_flag&PCCOLLECTION_LOAD ? 1740 : 1741)), sd.collection_list.size());
		clif_messagecolor(&sd.bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		status_calc_pc(&sd, SCO_FORCE);
	}
