...
static enum e_storage_add storage_canAddItem(struct s_storage *stor, int idx, struct item items[], int amount, int max_num) {
	...
	if (stor->stor_id == COLLECTION_STORAGE) {
		...
		if (!data->flag.collection) {
			sprintf(output, msg_txt(sd,1742), data->name.c_str(), data->ename.c_str(), data->nameid);
		...
		if (i < stor->max_amount) {
			sprintf(output, msg_txt(sd,1743), data->name.c_str(), data->ename.c_str(), data->nameid);
			clif_messagecolor(&sd->bl, color_table[COLOR_RED], output, false, SELF);
			sd->state.collection_flag |= PCCOLLECTION_RELOAD;
			return STORAGE_ADD_INVALID;
		}
