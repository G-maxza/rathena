...
int chat_joinchat(struct map_session_data* sd, int chatid, const char* pass)
{
	...
	if (cd->owner->type == BL_MOB) {
		...
		if (md->chat_clone_id) {
			...
			if (sd->status.char_id == md->chat_clone_char_id) {
				safesnprintf( output, CHAT_SIZE_MAX, "%s : %s", md->name, md->chat_clone_message );
				msg_len = (unsigned short)(strlen(output) + 1);
				WFIFOHEAD(sd->fd, msg_len + 12);
				WFIFOW(sd->fd, 0) = 0x2C1;
				WFIFOW(sd->fd, 2) = msg_len + 12;
				WFIFOL(sd->fd, 4) = 0;
				WFIFOL(sd->fd, 8) = (battle_config.chat_msg_color & 0x0000FF) << 16 | (battle_config.chat_msg_color & 0x00FF00) | (battle_config.chat_msg_color & 0xFF0000) >> 16;
				safestrncpy((char*)WFIFOP(sd->fd,12), output, msg_len);
				WFIFOSET(sd->fd, msg_len + 12);
			} else {
				safesnprintf(output, CHAT_SIZE_MAX, "%s", md->chat_clone_message);
				clif_wis_message(sd, md->name,  output, strlen(output) + 1, 0);
			}
