...
#if PACKETVER >= 20150513
struct PACKET_ZC_CONNECTION_REWARD {
	int16 PacketType;
	uint8 state;
	uint8 type;
	uint32 amount;
...
DEFINE_PACKET_HEADER(ZC_CONNECTION_REWARD, 0x0a15);

struct PACKET_CZ_CONNECTION_REWARD {
	int16 PacketType;
	char feature_name[NAME_LENGTH];
} __attribute__((packed));
DEFINE_PACKET_HEADER(CZ_CONNECTION_REWARD, 0x0a16);
#endif
