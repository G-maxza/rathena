...
int chrif_save(struct map_session_data *sd, int flag) {
	...
	if(battle_config.feature_connection_reward){
		pc_setreg2(sd,"#coRewardPts", sd->connection_reward_pts);
		if(sd->connection_reward_tick > 0)
			pc_setreg2(sd,"#coRewardTime", sd->connection_reward_tick - time(NULL));
		if(flag&CSAVE_QUIT && sd->connection_reward_timer != INVALID_TIMER){
			delete_timer(sd->connection_reward_timer, feature_connection_reward_timeout);
...
void chrif_parse_ack_vipActive(int fd) {
	...
	time_t now = time(NULL); // VIP System
	bool changed = false;
	...
	// VIP System
	if (sd->vip.time <= now)
		clif_status_change(&sd->bl, EFST_VIP_SYSTEM, 0, INVALID_TIMER, 0, 0, 0);
	...
	if ((flag&0x2)) //isgm
		...
		if((flag&0x1)) { //isvip
			...
			clif_status_change(&sd->bl, EFST_VIP_SYSTEM, 1, INFINITE_TICK, 0, 0, 0); // VIP System
			...
			clif_status_change(&sd->bl, EFST_VIP_SYSTEM, 0, INVALID_TIMER, 0, 0, 0); // VIP System
		}
	}
